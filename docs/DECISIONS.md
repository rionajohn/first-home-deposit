...
## D137. Frame 01 greets the participant by name, and the name is scenario data rather than a design element

**Date.** 1 September 2026. Adds one paragraph to frame 01 and one file to `src/`. Changes no figure,
no route, no guard and no heading.

### The greeting

`/home` opens with "Hi Finn" above the balance card, the affordance a bank's own account home carries.
The string is `content['/home'].greetingTemplate`, `'Hi {name}'`, and the screen fills `{name}` the way
`transactionRow` already fills `amountTemplate` from `MOCK_POSITION.moneyIn` (D58). Nothing about the
participant is typed into `content.js`.

### The name is a property of the seeded persona, not of the design

**The whole point of this entry.** "Finn" is not copy. It is who the session is run as, the same kind
of fact as the £450,000 property and the £8,950 already saved. So it lives in a new `src/persona.js`,
and **a scenario change is a data edit to one line** - `persona.name`. Nothing else in the codebase
contains the name, in code or in a comment; the comments that explain the split were deliberately
written not to quote the current value, so a rename cannot leave a stale one behind.

`state.js` was checked first and carries no persona fields: it holds the `build-spec.md` section 6
variables and the journey flags, every one of which a participant can change mid-session and every one
of which persists to `sessionStorage`. The name is neither - it is fixed for the study and never
written back - so putting it there would have made it the only non-figure in that store. `model/`
holds calculations and the figures they run on, and nothing derives from a name. `config.js` owns what
the *product* is called, which its own header is explicit is not the same question as who is using it.

### It is a paragraph, and the heading order is unchanged

Frame 01's only heading is the app bar's `<h1>`; its balance label, its "Today"/"Yesterday" section
headers and its entry card title are all `<p>`. Every other screen uses `<h2 class="screen-title">`
for its own headline, so an `<h2>` here would have put a salutation in the slot that holds "Your
savings goals" on `/goals` and "What we can see" on `/position`, ranking "Hi Finn" above the entry
card in the outline. Measured after the change: the screen still exposes exactly one heading, `h1`
"Home". No level is introduced, so no level can be skipped.

Styled `--text-title3` at weight 600 for the same reason - addressed rather than titular, and visibly
not the screen's title.

### What it displaced, measured rather than assumed

At 390x844 the greeting occupies 25px plus `.screen-content`'s 16px flex gap, so **everything below it
moves down 41 logical pixels.** Measured against the tab bar at y=788:

| | Before | After |
| --- | --- | --- |
| Balance card top | 80 | 121 |
| Entry card title | 712 to 762, **fully visible** | 753 to 803, **crosses the fold** |
| Entry card CTA top | 852, **below the fold** | 893, below the fold |

**The primary element was already below the fold before this change.** `entryCardCta` - the control
that starts the journey the whole study observes - has always needed a scroll on this viewport, and
this entry does not create that. What it does create is the loss of the entry card title's second
line: "Thinking about buying a house?" was fully readable without scrolling and now clips after
"buying a". Accepted deliberately rather than absorbed by tightening the surrounding card margins,
because reclaiming 41px from frame 01's spacing would change a layout the reference PNG fixes in order
to protect an element that is off-screen either way.

**Logged as `GAPS.md` G131, not fixed here:** frame 01's call to action is not visible at 390x844 in
either state. That predates the greeting and is a question about the frame, not about this paragraph.

### Not extended to any other screen, and why that does not read as unfinished

A name on the bank's account home and nowhere else is the standard pattern, not a half-applied one:
the greeting belongs to the bank's own root screen, and every feature screen sits *inside* "Your first
home" rather than beside it. The one to watch is `/goals`, the other reachable tab root - if it ever
gains a salutation the two become a pattern, and its absence on `/tracker` would then read as missing.
No other screen changes.

### Cost of reversal

Delete `src/persona.js`, its line in `sw.js`'s `SHELL_ASSETS`, `greetingTemplate`, the `.home-greeting`
rule and the one paragraph in `home.js`. Nothing derives from any of them.

`CACHE_VERSION` and `BUILD_VERSION` v118 to v119.
